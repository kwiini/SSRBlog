...
async function vectorSearch(
  ...
  const hits = searchByVector(queryEmbedding, { topK, threshold: HYBRID_CONFIG.vectorThreshold })
  const chunkById = new Map(chunks.map(c => [c.id, c]))
  return hits
    .map(h => ({ id: h.id, score: h.similarity, chunk: chunkById.get(h.id)! }))
    .filter(r => r.chunk)
}
...
export async function hybridSearch(
  ...
  const vectorResults = await vectorSearch(query, chunks, HYBRID_CONFIG.rerankTopK)
