 ...
 */
function calculateArticleAverageEmbedding(chunks: VectorStore['chunks']): number[] {
 ...
 * @returns 相关文章列表
 */
async function getRelatedPosts(currentPath: string, topK: number = 3): Promise<RelatedPost[]> {
  ...
  // 获取当前文章的所有 chunks
  const currentArticleChunks = store.chunks.filter(chunk => chunk.source === currentPath)
  ...
  // 计算当前文章的平均向量
  const currentArticleEmbedding = calculateArticleAverageEmbedding(currentArticleChunks)

  // 按文章分组计算相似度
  const articleSimilarities = new Map<string, {
    title: string
    path: string
