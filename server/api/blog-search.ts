 ...
 * @returns 搜索结果
 */
async function searchBlogs(query: string, topK: number = 5): Promise<SearchResult[]> {
 ...
 * @returns 推荐结果
 */
async function getRelatedArticles(currentPath: string, topK: number = 3): Promise<SearchResult[]> {
  const store = await loadVectors()
  ...
  // 计算与其他文章的相似度
  const articleScores = new Map<string, { title: string; path: string; score: number; count: number }>()
