...
async function scoreContextPrecision(
  ...
  const result = await judgeJSON<{ relevance: number[] }>(
    caller,
    prompt,
    { relevance: [] },
  )
...
async function scoreContextRecall(
  ...
  const result = await judgeJSON<{ recall: number }>(
    caller,
    prompt,
    { recall: 0 },
  )
...
async function scoreFaithfulness(
  ...
  const result = await judgeJSON<{ claims: { supported: boolean }[]; score: number }>(
    caller,
    prompt,
    { claims: [], score: 0 },
  )
...
async function generateAnswer(prompt: string, caller: LLMCaller): Promise<string> {
  try {
