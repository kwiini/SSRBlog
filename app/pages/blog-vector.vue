  ...
  <div>
    ...
    <div class="bg-white border border-stone-200/60 rounded-2xl p-6 mb-6 shadow-sm">
      <div class="flex items-center justify-between mb-5">
        ...
        >
          <span class="w-1.5 h-1.5 rounded-full" :class="stats?.exists ? 'bg-emerald-500' : 'bg-amber-500'"></span>
          {{ stats?.exists ? '已就绪' : '未生成' }}
        </span>
      </div>

      <div v-if="stats?.exists" class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-stone-50 rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-stone-800">{{ stats.articleCount }}</p>
          <p class="text-xs text-stone-500 mt-1">文章数量</p>
        </div>
        <div class="bg-stone-50 rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-stone-800">{{ stats.chunkCount }}</p>
          <p class="text-xs text-stone-500 mt-1">Chunk 数量</p>
        </div>
        <div class="bg-stone-50 rounded-xl p-4 text-center">
          <p class="text-sm font-semibold text-stone-700 mt-1">{{ formatDate(stats.lastUpdated) }}</p>
          <p class="text-xs text-stone-500 mt-1">最后更新</p>
        </div>
      </div>

      <div v-else class="bg-stone-50 rounded-xl p-6 text-center mb-6">
        <div class="w-12 h-12 bg-stone-200 rounded-xl flex items-center justify-center mx-auto mb-3">
      ...
      <div class="flex gap-3">
        ...
        >
          <svg v-if="loading.vectorize" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white border border-stone-200/60 rounded-2xl p-6 mb-6 shadow-sm">
      ...
      <div class="relative mb-5">
        ...
        >
          <svg v-if="loading.search" class="w-3.5 h-3.5 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
          </svg>
        ...
        <div v-if="searchResults.length > 0" class="space-y-3">
          ...
          >
            <div class="flex items-start justify-between gap-4 mb-2">
              ...
              <span class="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full shrink-0">
      ...
      <Transition name="fade">
        <div v-if="searched && searchResults.length === 0" class="text-center py-10">
      ...
      <div v-if="stats?.articles?.length" class="bg-white border border-stone-200/60 rounded-2xl p-6 shadow-sm">
        <div class="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p class="text-xs text-stone-500">共 {{ stats.articles.length }} 篇文章</p>
          </div>
        </div>
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto pr-2">
          ...
          >
            ...
            <span class="text-xs text-stone-400 bg-stone-100 px-2 py-0.5 rounded-full shrink-0">
    ...
    <Transition name="toast">
      ...
      >
        ...
        {{ error }}
        <button @click="error = ''" class="ml-2 text-stone-400 hover:text-white transition-colors">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
