<template>
  <div>
    ...
    >
      <div class="w-8 h-8 rounded-lg bg-stone-100 flex items-center justify-center mr-3 group-hover:bg-stone-200 transition-colors">
    ...
    <!-- 文章卡片 -->
    <article class="bg-white border border-stone-200/60 rounded-2xl overflow-hidden shadow-sm mb-8">
      <!-- 文章头部 - 改进设计 -->
      <div class="px-8 py-8 border-b border-stone-100 bg-linear-to-br from-stone-50/50 to-white">
        <!-- 标签 -->
        <div v-if="post.meta?.tags" class="flex flex-wrap gap-2 mb-4">
          <span 
        ...
        <h1 class="text-2xl md:text-3xl font-bold text-stone-800 mb-4 leading-tight">
          {{ post.title }}
        </h1>
        
        <div class="flex flex-wrap items-center gap-5 text-sm text-stone-500">
          <div class="flex items-center">
            <div class="w-8 h-8 rounded-full bg-linear-to-br from-stone-700 to-stone-800 flex items-center justify-center mr-2.5">
      ...
      <div class="px-8 py-6 border-t border-stone-100 bg-stone-50/50">
        <div class="flex items-center justify-between">
          ...
          >
            <div class="w-8 h-8 rounded-lg bg-white border border-stone-200 flex items-center justify-center mr-2.5 group-hover:border-stone-300 transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
              </svg>
            </div>
    ...
    <Transition name="fade">
      <div v-if="relatedPosts.length > 0" class="bg-white border border-stone-200/60 rounded-2xl p-6 shadow-sm">
        <div class="flex items-center gap-3 mb-5">
          <div class="w-10 h-10 rounded-xl bg-linear-to-br from-amber-500 to-orange-500 flex items-center justify-center shadow-sm">
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div class="flex-1 min-w-0">
                <h3 class="text-sm font-semibold text-stone-800 mb-1 group-hover:text-emerald-600 transition-colors line-clamp-1">
                  {{ related.title }}
                </h3>
                <p v-if="related.description" class="text-xs text-stone-500 line-clamp-2 leading-relaxed">
                  {{ related.description }}
                </p>
              </div>
              <div class="flex items-center gap-2 shrink-0">
                <div class="flex items-center gap-1 px-2 py-1 bg-stone-100 rounded-lg">
                  <svg class="w-3 h-3 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m16.94 0l-.707.707"/>
                  </svg>
                  <span class="text-xs font-medium text-stone-600">{{ (related.similarity * 100).toFixed(0) }}%</span>
                </div>
