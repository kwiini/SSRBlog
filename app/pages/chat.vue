<template>
  <div class="h-[calc(100vh-220px)] flex flex-col">
    ...
    >
      <div class="flex items-center gap-4">
        <div
          ...
          :class="
            useRAG
              ? 'bg-linear-to-br from-emerald-500 to-emerald-700 shadow-emerald-200'
              : 'bg-linear-to-br from-stone-700 to-stone-900 shadow-stone-200'
          "
        >
          <svg
    ...
    <div
      ...
      class="flex-1 overflow-y-auto py-6 space-y-5 scroll-smooth"
    >
      <!-- 欢迎消息 -->
      <Transition name="fade-scale">
        ...
        >
          ...
          <h3 class="text-stone-700 font-medium text-lg mb-2">
            {{ useRAG ? "开始知识库对话" : "开始 AI 对话" }}
          </h3>
          <p class="text-sm text-stone-500 max-w-sm mx-auto leading-relaxed">
            {{
              useRAG
                ? "基于博客知识库，为你提供精准的智能问答服务"
                : "与 AI 助手自由交流，探索无限可能"
            }}
          </p>
          <!-- 快捷提示 -->
          ...
          >
            ...
            <div class="flex flex-col min-w-0">
              ...
              <Transition name="fade">
                ...
                >
                  <p
                    class="text-xs text-emerald-600 mb-1.5 font-medium flex items-center gap-1"
                  >
                    <svg
                      class="w-3 h-3"
                      fill="none"
              ...
              >
                <div
                  ...
                  class="prose prose-sm max-w-none prose-stone"
                >
              ...
              <!-- 时间戳 -->
              <span
                class="text-[11px] mt-1.5 text-stone-400 font-medium"
                :class="
      ...
      <Transition name="fade">
        ...
        >
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-xl bg-linear-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-sm"
            >
              <span class="text-xs font-semibold text-white">AI</span>
            </div>
            <div
              class="bg-white border border-stone-200 rounded-2xl rounded-tl-sm px-4 py-3 shadow-sm"
            >
              <div class="flex items-center gap-1.5">
                ...
                />
                <div
                  class="w-2 h-2 bg-emerald-400 rounded-full animate-bounce delay-100"
                />
                <div
                  class="w-2 h-2 bg-emerald-400 rounded-full animate-bounce delay-200"
                />
              </div>
            </div>
          </div>
    ...
    <div class="border-t border-stone-200/60 pt-5">
      <div class="relative group">
        <div
          class="absolute -inset-0.5 bg-linear-to-r from-stone-200 to-stone-300 rounded-2xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-300 blur"
        ></div>
        <div
          class="relative flex items-end gap-2 bg-white border border-stone-200 rounded-xl p-2 focus-within:border-stone-400 focus-within:shadow-lg transition-all duration-300"
        >
          <textarea
            v-model="inputMessage"
            rows="1"
            :placeholder="
              useRAG ? '输入问题，基于博客知识库回答...' : '输入消息开始对话...'
            "
      ...
      <p
        class="text-xs text-stone-400 mt-3 text-center flex items-center justify-center gap-2"
      >
        <span class="flex items-center gap-1">
          <kbd
            class="px-1.5 py-0.5 bg-stone-100 rounded text-[10px] font-sans border border-stone-200"
            >Enter</kbd
          >
          发送
        </span>
...
const formatMessage = (content) => {
  ...
  formatted = formatted.replace(
    /```([\s\S]*?)```/g,
    '<pre class="bg-stone-100 rounded-lg p-3 overflow-x-auto my-2 text-xs"><code>$1</code></pre>',
  );

  // 行内代码
  formatted = formatted.replace(
    /`([^`]+)`/g,
    '<code class="bg-stone-100 px-1.5 py-0.5 rounded text-xs text-stone-700">$1</code>',
  );
