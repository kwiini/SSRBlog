  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-sm text-stone-500 mt-1">管理博客文章，支持编辑、删除和重新向量化</p>
      </div>
      <NuxtLink 
        to="/admin/write"
    ...
    <div class="grid grid-cols-3 gap-4 mb-8">
      ...
      <div class="bg-white border border-stone-200/60 rounded-xl p-5 shadow-sm">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-2xl font-bold text-stone-800">{{ vectorStats?.articleCount || 0 }}</p>
      ...
      <div class="bg-white border border-stone-200/60 rounded-xl p-5 shadow-sm">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-semibold text-stone-700">{{ formatDate(vectorStats?.lastUpdated) || '未生成' }}</p>
    ...
    <div class="bg-white border border-stone-200/60 rounded-xl shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
        ...
        >
          <svg v-if="regenerating" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="divide-y divide-stone-100">
        ...
        >
          <div class="flex-1 min-w-0 mr-4">
            <div class="flex items-center gap-3 mb-1">
              <h3 class="font-medium text-stone-800 truncate">{{ post.title }}</h3>
              <span 
            ...
            <p class="text-sm text-stone-500 truncate">{{ post.description || '暂无描述' }}</p>
            <p class="text-xs text-stone-400 mt-1">{{ formatDate(post.meta?.date) }}</p>
          </div>
        ...
        <div v-if="posts.length === 0" class="px-6 py-12 text-center">
          <div class="w-16 h-16 bg-stone-100 rounded-xl flex items-center justify-center mx-auto mb-4">
