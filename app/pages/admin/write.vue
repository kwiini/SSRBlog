  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-semibold text-stone-800">{{ isEditing ? '编辑文章' : '写文章' }}</h1>
          <p class="text-sm text-stone-500 mt-1">{{ isEditing ? '修改文章内容并更新向量' : '创建新文章并自动生成向量' }}</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        >
          <svg v-if="vectorizing" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-3 gap-6">
      ...
      <div class="col-span-2 space-y-4">
        <!-- 标题输入 -->
        <div class="bg-white border border-stone-200/60 rounded-xl p-4 shadow-sm">
          <label class="block text-sm font-medium text-stone-700 mb-2">文章标题</label>
          <input 
            v-model="post.title"
            type="text"
            placeholder="输入文章标题..."
        ...
        <div class="bg-white border border-stone-200/60 rounded-xl shadow-sm overflow-hidden">
          <div class="flex items-center justify-between px-4 py-3 border-b border-stone-100">
            <label class="text-sm font-medium text-stone-700">文章内容 (Markdown)</label>
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white border border-stone-200/60 rounded-xl p-5 shadow-sm">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-stone-700 mb-2">文章路径</label>
              <div class="flex items-center gap-2">
              ...
              <p class="text-xs text-stone-400 mt-1">用于 URL，只能包含字母、数字和连字符</p>
            </div>

            <div>
              <label class="block text-sm font-medium text-stone-700 mb-2">发布日期</label>
              <input 
                v-model="post.date"
                type="date"
            ...
            <div>
              <label class="block text-sm font-medium text-stone-700 mb-2">标签</label>
              <input 
                v-model="post.tags"
                type="text"
        ...
        <!-- 描述 -->
        <div class="bg-white border border-stone-200/60 rounded-xl p-5 shadow-sm">
          <label class="block text-sm font-medium text-stone-700 mb-2">文章描述</label>
          <textarea 
          ...
          ></textarea>
          <p class="text-xs text-stone-400 mt-2">显示在文章列表中，建议 100-200 字</p>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-blue-600 mt-1">发布后文章将自动进行向量化，用于智能搜索和相关文章推荐功能。</p>
            </div>
          </div>
        </div>

        <!-- 向量化进度 -->
        <div v-if="vectorizing" class="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
          <div class="flex items-center gap-3">
    ...
    <Transition name="toast">
      <div 
        ...
        :class="toast.type === 'success' ? 'bg-stone-800 text-white' : 'bg-red-600 text-white'"
      >
...
tags: [${post.value.tags.split(',').map(t => `'${t.trim()}'`).join(', ') || ''}]
---
  ...
  try {
    ...
    if (data) {
      post.value = {
        ...
        tags: Array.isArray(data.meta?.tags) ? data.meta.tags.join(', ') : (data.meta?.tags || '')
      }
...
const savePost = async (publish = false) => {
  const articleData = {
    ...
    date: post.value.date,
    tags: post.value.tags.split(',').map(t => t.trim()).filter(Boolean),
    publish
  }
